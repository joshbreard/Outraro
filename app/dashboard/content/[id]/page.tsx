...
export default async function ContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!item) {
    return (
      <div className="max-w-3xl text-center py-20">
        <h1 className="text-2xl font-bold text-surface-900 mb-4">Content not found</h1>
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/dashboard" className="inline-flex items-center gap-1 text-surface-500 hover:text-surface-700 text-sm mb-6 transition-colors">
      ...
      {item.imageUrl && (
        <div className="relative w-full aspect-[2/1] rounded-2xl overflow-hidden mb-8">
          <Image src={item.imageUrl} alt={item.title} fill className="object-cover" sizes="100vw" priority />
        </div>
      )}
      ...
      <h1 className="text-3xl font-bold text-surface-900 mb-4">{item.title}</h1>
      {item.summary && <p className="text-lg text-surface-500 leading-relaxed mb-8">{item.summary}</p>}

      <div className="border-t border-surface-200 pt-8" dangerouslySetInnerHTML={{ __html: bodyHtml }} />
    </div>
  );
}
