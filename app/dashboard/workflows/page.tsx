...
export default async function WorkflowsPage() {
  ...
  const content = allContent.filter((item) => item.category === "Workflow");

  return (
    <div className="max-w-4xl">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-surface-900 mb-2">Workflows</h2>
        <p className="text-surface-500 text-sm">Plug-and-play automation templates you can deploy today.</p>
      </div>
      ...
      {content.length === 0 ? (
        ...
        <div className="space-y-4">
          {content.map((item) => (
            <Link key={item.id} href={`/dashboard/workflows/${item.id}`} className="block no-underline">
